...
def add(
        ...
        rank: Annotated[
            int, typer.Option("-r", "--rank",
                              help="Rank of the server in which to add the target")],
        type: Annotated[
            str, typer.Option("-t", "--type",
                              help="Type of target")] = "map",
        groupfile: Annotated[
            str, typer.Option("-g", "--groupfile",
                              help="Flock group file of the service")] = "./mofka.json"
        ):
    from ._util import parse_config_from_args
    target_config = parse_config_from_args(ctx.args.copy(), "--config.")
    target_config.update(parse_config_from_args(ctx.args.copy(), "--c."))
    target_dependencies = parse_config_from_args(ctx.args.copy(), "--dependencies.")
    target_dependencies.update(parse_config_from_args(ctx.args.copy(), "--d."))
    ...
    for k, v in target_dependencies.items():
        ...
        if isinstance(v, list):
            for e in v:
                ...
                print(f"Error: invalid dependency specifications {target_dependencies}")
